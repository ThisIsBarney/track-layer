// language=GLSL
export default `
#define SHADER_NAME track-layer-vertex-64-shader
...
void main() {
  ...
  vec3 startPos = vec3(instanceStartPositions.xy, 0.1);
  vec3 endPos = vec3(instanceEndPositions.xy, 0.1);

  // Calculate current position 64bit

  vec3 currPosition = mix(startPos, endPos, isEnd);
  ...
  vec3 prevPosition = mix(-instanceLeftDeltas, vec3(0.0, 0.0, 0.1), isEnd) + startPos;
  ...
  vec3 nextPosition = mix(vec3(0.0), instanceRightDeltas, isEnd) + endPos;
  ...
  float currentZ = mix(instanceStartPositions.z, instanceEndPositions.z, positions.x);

  vX = 1.0 - (currentTime - currentZ) / trailLength;
  
  invalid = 0.0;

  float elapse = instanceEndPositions.z - instanceStartPositions.z;
